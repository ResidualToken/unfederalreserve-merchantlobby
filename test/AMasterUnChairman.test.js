...
contract('MasterUnChairman', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.sushi = await UnFederalReserveToken.new({ from: alice });
    });

    it('should set correct state variables', async () => {
        this.chef = await MasterUnChairman.new(this.sushi.address, dev, '1000', '0', '1000', { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await MasterUnChairman.new(this.sushi.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should not distribute SUSHIs if no one deposit', async () => {
            ...
            this.chef = await MasterUnChairman.new(this.sushi.address, dev, '100', '200', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            this.chef = await MasterUnChairman.new(this.sushi.address, dev, '100', '300', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            //   MasterUnChairman should have the remaining: 10000 - 5666 = 4334
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await MasterUnChairman.new(this.sushi.address, dev, '100', '400', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.chef.pendingUnFederalReserve(0, alice)).valueOf(), '10000');
            // Bob deposits 10 LP2s at block 425
            await time.advanceBlockTo('424');
            await this.chef.deposit(1, '5', { from: bob });
            // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
            assert.equal((await this.chef.pendingUnFederalReserve(0, alice)).valueOf(), '11666');
            await time.advanceBlockTo('430');
            // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
            assert.equal((await this.chef.pendingUnFederalReserve(0, alice)).valueOf(), '13333');
            assert.equal((await this.chef.pendingUnFederalReserve(1, bob)).valueOf(), '3333');
        });

        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            // 100 per block farming rate starting at block 500 with bonus until block 600
            this.chef = await MasterUnChairman.new(this.sushi.address, dev, '100', '500', '600', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.chef.pendingUnFederalReserve(0, alice)).valueOf(), '10500');
            // At block 606, Alice withdraws all pending rewards and should get 10600.
            await this.chef.deposit(0, '0', { from: alice });
            assert.equal((await this.chef.pendingUnFederalReserve(0, alice)).valueOf(), '0');
