...
const UnFederalReserveBar = artifacts.require('UnFederalReserveBar');

contract('UnFederalReserveToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.sushi = await UnFederalReserveToken.new({ from: alice });
        this.bar = await UnFederalReserveBar.new(this.sushi.address, { from: alice });
    ...
    it('should work with more than one participant', async () => {
        ...
        // UnFederalReserveBar get 20 more SUSHIs from an external source.
